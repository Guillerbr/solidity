...
- If a contract receives Ether (without a function being called),
  either the receive or the fallback function is executed.
...
- If you want to send Ether using ``address.transfer``, there are certain details to be aware of:
  ...
  3. Sending Ether can also fail because the execution of the recipient
     ...
     using ``require``, ``assert``, ``revert``, ``throw`` or because the
     operation is just too expensive) - it "runs out of gas" (OOG).  If you
